...
def generate_phone():
	# +7 xxx xxx xx xx
	random_phone = tuple(str(int(random.random()*10)) for __ in range(10))
...
def generate_email(max_part_size=8):
	...
	parts_sizes = [int(random.random()*max_part_size) + 1 for __ in range(3)]
...
def generate_text(name=False, max_text_size=120):
	# Max size 120 sumbols if text and 8 if name
